#import "frontmatter.typ": orcidLogo, emailIcon, loadFrontmatter
...
#let template(
  ...
  // Figure caption alignment and font size.
  caption-align: left,
  caption-fontsize: 8pt,
  // The paper's content.
  body
) = {
  ...
  show heading: it => context {
    ...
    if it.level == 1 [
      ...
      // Second-level headings are bold run-ins.
      ...
      // Third level headings are bold italic run-ins.
  ...
  let has-equal-contributors = authors.any(author => "equal_contributor" in author and author.equal_contributor == true)
  if authors.len() > 0 {
    box(inset: (y: 10pt), {
      authors.map(author => {
        text(11pt, weight: "semibold", author.name)
        if "equal_contributor" in author and author.equal_contributor == true {
          super("*")
        }
        h(1pt)
        ...
        if "corresponding" in author and author.corresponding == true and "email" in author {
          emailIcon(email: author.email)
        }
      }).join(", ", last: ", and ")
    })
  }
  ...
  if has-equal-contributors {
    block(inset: (bottom: 8pt), text(size: 8pt, style: "italic", [\*These authors contributed equally to this work.]))
  }
...
if (abstracts != none and abstracts.len() > 0) {
  ...
  show figure.caption: set text(size: caption-fontsize)
